...
const BillingChart = ({ dateRange, onDrillDown }) => {
  const { monthlyBilling, billingTotals } = useKamDataContext();

  const filteredData = monthlyBilling.slice(dateRange[0], dateRange[1] + 1).map(d => ({
    ...d,
  ...
  const handleClick = (data) => {
    if (data && data.activePayload) {
      onDrillDown?.('billing', data.activePayload[0]?.payload);
    }
  };
  ...
  return (
    <div className="chart-card">
      <div className="chart-header">
        <div>
          <h3>📋 On-Time Billing (INR Cr)</h3>
          <p className="chart-subtitle">Monthly target vs achievement with % overlay</p>
        </div>
        <div className="chart-actions">
          <span className="ytd-badge">
            YTD: {formatCrore(billingTotals.totalAchievement)} / {formatCrore(billingTotals.totalTarget)}
            <span style={{ color: getAchievementColor(billingTotals.achievementPercentage), marginLeft: 8 }}>
              ({(billingTotals.achievementPercentage * 100).toFixed(1)}%)
            </span>
          </span>
          <button
            className="export-btn"
            onClick={() => downloadCSV(monthlyBilling, 'billing_data.csv')}
            title="Export CSV"
          >
            📥 CSV
          </button>
      ...
      <ResponsiveContainer width="100%" height={350}>
        <ComposedChart data={filteredData} onClick={handleClick}>
          ...
          <Legend />
          <ReferenceLine yAxisId="left" y={25} stroke="#94a3b8" strokeDasharray="5 5" label={{ value: 'Target Line', position: 'right', fontSize: 10, fill: '#94a3b8' }} />
          <Area yAxisId="left" type="monotone" dataKey="achievement" fill="url(#billingGrad)" stroke="none" name="Achievement Area" />
          <Bar yAxisId="left" dataKey="target" fill="#6366f1" opacity={0.3} name="Target" radius={[4, 4, 0, 0]} barSize={30} />
          <Bar yAxisId="left" dataKey="achievement" fill="#10b981" name="Achievement" radius={[4, 4, 0, 0]} barSize={30} />
          <Line yAxisId="right" type="monotone" dataKey="percentDisplay" stroke="#f59e0b" strokeWidth={2.5} dot={{ r: 5, fill: '#f59e0b' }} activeDot={{ r: 7 }} name="Achievement %" connectNulls={false} />
        </ComposedChart>
      </ResponsiveContainer>
    </div>
  );
